...
# SELF-CONTAINED: product scripts (card.py, hourly_extractor.py, ...) are vendored
# read-only under lib/product_scripts_ro/, so this folder runs without the product.
SCRIPTS = BENCH_DIR / "lib" / "product_scripts_ro"
CARD = SCRIPTS / "card.py"

# SAFETY: the study NEVER reads or writes any LIVE board. Recall runs entirely
# against a frozen local COPY (board_snapshot.json) so the harness cannot
# interfere with any running product, and numbers don't drift. Re-snapshot
# deliberately: cp <product>/board/board.json board_snapshot.json
