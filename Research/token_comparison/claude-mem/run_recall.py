...
def run(cm_params: dict | None = None) -> dict:
    ...
    def agg(subset):
        ...
        wmean, cmean = statistics.mean(wbt), statistics.mean(cmt)
        return {
            "n": len(subset),
...
def _print(out: dict):
    ...
    print(f"  reduction      : {a['reduction_pct']}%  (WB wins {a['wb_wins']}, CM wins {a['cm_wins']})")
    for sh, s in out["by_shape_found_only"].items():
        if s:
            print(f"  {sh:9}: WB {s['wb_mean_total']:.0f} vs CM {s['cm_mean_total']:.0f}  "
                  f"= {s['reduction_pct']}%  (WB {s['wb_wins']}/{s['n']})")
